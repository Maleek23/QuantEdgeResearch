...
export function InsightPanel({ className, children }: InsightPanelProps) {
  return (
    <div className={cn(
      "bg-muted/50 backdrop-blur-md border border-border/40 rounded-lg p-4 mt-4",
      className
    )}>
      {children}
    </div>
...
export function ConfluenceInsights({ 
  ...
  const getGrade = (s: number) => {
    if (s >= 95) return { grade: 'A+', tier: 'Exceptional', color: 'text-[var(--trade-bullish)]' };
    if (s >= 90) return { grade: 'A', tier: 'Excellent', color: 'text-[var(--trade-bullish)]' };
    if (s >= 85) return { grade: 'A-', tier: 'Very Strong', color: 'text-[var(--trade-bullish)]' };
    if (s >= 80) return { grade: 'B+', tier: 'Strong', color: 'text-cyan-400' };
    if (s >= 75) return { grade: 'B', tier: 'Good', color: 'text-cyan-400' };
    if (s >= 70) return { grade: 'B-', tier: 'Moderate', color: 'text-[var(--trade-neutral)]' };
    if (s >= 65) return { grade: 'C+', tier: 'Caution', color: 'text-[var(--trade-neutral)]' };
    if (s >= 60) return { grade: 'C', tier: 'Weak', color: 'text-[var(--trade-bearish)]' };
    return { grade: 'D', tier: 'Avoid', color: 'text-[var(--trade-bearish)]' };
  };
  ...
  const rsiStatus = rsi >= 70 ? { text: 'Overbought', color: 'text-[var(--trade-neutral)]', pts: -5 } :
    rsi <= 30 ? { text: 'Oversold', color: 'text-[var(--trade-neutral)]', pts: -5 } :
    { text: 'Healthy', color: 'text-[var(--trade-bullish)]', pts: 8 };
  
  const volStatus = atrPercent <= 1 ? { text: 'Low Vol', color: 'text-[var(--trade-bullish)]', pts: 8 } :
    atrPercent <= 2 ? { text: 'Moderate', color: 'text-[var(--trade-neutral)]', pts: 0 } :
    { text: 'High Vol', color: 'text-[var(--trade-bearish)]', pts: -5 };
  ...
  const levelStatus = priceToResistance <= 2 ? { text: 'Near Resistance', color: 'text-[var(--trade-neutral)]', pts: -2 } :
    priceToResistance >= 10 ? { text: 'Room to Run', color: 'text-[var(--trade-bullish)]', pts: 5 } :
    { text: 'Mid-Range', color: 'text-foreground/80', pts: 0 };
  ...
  return (
    <InsightPanel>
      ...
      <div className="space-y-2 mb-4">
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between items-center text-xs">
            ...
            <span className={biasAligned ? "text-[var(--trade-bullish)]" : "text-[var(--trade-neutral)]"}>
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span className={trend === 'up' ? "text-[var(--trade-bullish)]" : trend === 'down' ? "text-[var(--trade-bearish)]" : "text-[var(--trade-neutral)]"}>
      ...
      <div className="bg-card/50 rounded-lg p-3 mb-4">
        <h5 className="text-xs font-semibold text-foreground/90 mb-2 flex items-center gap-1.5">
          <Info className="h-3 w-3 text-cyan-400" />
          CONFIDENCE INTERPRETATION
        </h5>
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="flex justify-between">
            <span className="text-muted-foreground">60-69% (C+)</span>
            <span className="text-[var(--trade-bearish)]">Weak - Avoid</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">70-79% (B/B+)</span>
            <span className="text-[var(--trade-neutral)]">Moderate - Small size</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">80-89% (A/A-)</span>
            <span className="text-cyan-400">Strong - Standard</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">90-100% (A+)</span>
            <span className="text-[var(--trade-bullish)]">Exceptional - Full size</span>
          </div>
        </div>
      </div>
    ...
    <InsightPanel>
      <div className="mb-4">
        <div className="flex items-center gap-2 mb-2">
          <span className={cn(
            "font-semibold",
            trend === 'up' ? 'text-[var(--trade-bullish)]' : trend === 'down' ? 'text-[var(--trade-bearish)]' : 'text-foreground/80'
          )}>
            {trend === 'up' ? 'Strong Uptrend' : trend === 'down' ? 'Downtrend' : 'Ranging/Neutral'}
          </span>
          <span className="text-muted-foreground">|</span>
          <span className="font-mono text-foreground/80 text-sm">{trendStrength}</span>
        </div>
        ...
        {rsiCondition === 'overbought' && (
          <p className="text-xs text-[var(--trade-neutral)]">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-[var(--trade-bullish)]/10 border border-green-500/30 rounded-lg p-3">
          <h5 className="text-[var(--trade-bullish)] font-semibold text-xs mb-1">SUPPORT: ${safeToFixed(support, 2)}</h5>
          <ul className="text-xs text-muted-foreground space-y-0.5">
            <li>Major support level</li>
            <li>{safeToFixed(priceToSupport, 1)}% below current price</li>
          </ul>
          <p className="text-[var(--trade-neutral)] text-xs mt-2">
            If broken: Expect accelerated selling
          </p>
        </div>
        
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
          <h5 className="text-[var(--trade-bearish)] font-semibold text-xs mb-1">RESISTANCE: ${safeToFixed(resistance, 2)}</h5>
          <ul className="text-xs text-muted-foreground space-y-0.5">
            <li>Key resistance zone</li>
            <li>{safeToFixed(priceToResistance, 1)}% above current</li>
          </ul>
          <p className="text-[var(--trade-bullish)] text-xs mt-2">
      ...
      <div className="mb-4">
        <h5 className="text-xs font-semibold text-foreground/90 mb-2">TRADE STRATEGIES BASED ON LEVELS</h5>
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-card/50 rounded p-2 border border-border/40">
            ...
            <div className="text-xs text-muted-foreground space-y-0.5 font-mono">
              ...
              <div className="text-[var(--trade-bullish)]">R:R: {breakoutRR}:1</div>
            </div>
          </div>
          ...
          <div className="bg-card/50 rounded p-2 border border-border/40">
            ...
            <div className="text-xs text-muted-foreground space-y-0.5 font-mono">
              ...
              <div className="text-[var(--trade-neutral)]">R:R: {rangeRR}:1</div>
            </div>
          </div>
          
          <div className="bg-card/50 rounded p-2 border border-border/40">
            <h6 className="text-[var(--trade-neutral)] font-semibold text-xs mb-1">Wait & Watch</h6>
  ...
  return (
    <InsightPanel>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h4 className="text-sm font-semibold text-foreground/90">POSITION SIZE CALCULATOR</h4>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div>
          ...
          <Input
            ...
            className="h-8 bg-card/50 border-border text-sm font-mono"
            data-testid="input-account-size"
          />
        </div>
        <div>
          <label className="text-xs text-muted-foreground block mb-1">Risk Per Trade %</label>
          <Input
            type="number"
            value={riskPercent}
        ...
        <div>
          <label className="text-xs text-muted-foreground block mb-1">Trade Risk</label>
          <div className="h-8 flex items-center font-mono text-sm text-foreground/90">
      ...
      <div className="bg-card/50 rounded-lg p-3 mb-4">
        ...
        {isOversized && (
          <div className="text-xs text-[var(--trade-neutral)] mt-2 flex items-center gap-1.5">
        ...
        {!isValidSetup && (
          <div className="text-xs text-[var(--trade-bearish)] mt-2 flex items-center gap-1.5">
      ...
      <div className="space-y-1.5 mb-4">
        <h5 className="text-xs font-semibold text-muted-foreground uppercase">Risk Scenarios</h5>
        <div className="flex justify-between text-xs">
          <span className="text-muted-foreground">If Stop Hit:</span>
        ...
        {targets[0] && (
          <div className="flex justify-between text-xs">
            <span className="text-muted-foreground">If Target 1 Hit:</span>
        ...
        {targets[1] && (
          <div className="flex justify-between text-xs">
            <span className="text-muted-foreground">If Target 2 Hit:</span>
            <span className="font-mono text-[var(--trade-bullish)]">
        ...
        {targets[2] && (
          <div className="flex justify-between text-xs">
            <span className="text-muted-foreground">If Target 3 Hit:</span>
            <span className="font-mono text-[var(--trade-bullish)]">
...
export function MarketContextInsights({
  ...
  const regimeInfo = {
    'trending': { label: 'Trending', color: 'text-[var(--trade-bullish)]', strategy: 'Momentum strategies work best. Follow the trend, avoid mean reversion.' },
    'ranging': { label: 'Ranging', color: 'text-[var(--trade-neutral)]', strategy: 'Mean reversion strategies win 58% vs momentum 42%. Short at resistance, long at support.' },
    'volatile': { label: 'Volatile', color: 'text-[var(--trade-bearish)]', strategy: 'Reduce position sizes. Use wider stops but smaller positions.' },
    'neutral': { label: 'Neutral', color: 'text-foreground/80', strategy: 'No clear edge. Consider sitting out or using very small size.' }
  }[regime?.toLowerCase()] || { label: regime, color: 'text-foreground/80', strategy: '' };
  
  return (
    <InsightPanel>
      <div className="flex items-center gap-2 mb-4">
        <Clock className="h-4 w-4 text-cyan-400" />
        <h4 className="text-sm font-semibold text-foreground/90">MARKET CONTEXT</h4>
        <Badge variant="outline" className={shouldTrade ? "text-[var(--trade-bullish)] border-green-500/50" : "text-[var(--trade-neutral)] border-amber-500/50"}>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-card/50 rounded-lg p-3">
          <h5 className="text-xs font-semibold text-muted-foreground uppercase mb-1">Session</h5>
          <div className="text-sm font-semibold text-foreground/90">{sessionInfo.label}</div>
          <p className="text-xs text-muted-foreground mt-1">{sessionInfo.advice}</p>
        </div>
        
        <div className="bg-card/50 rounded-lg p-3">
          <h5 className="text-xs font-semibold text-muted-foreground uppercase mb-1">Regime</h5>
          <div className={cn("text-sm font-semibold", regimeInfo.color)}>{regimeInfo.label}</div>
          <p className="text-xs text-muted-foreground mt-1">{regimeInfo.strategy}</p>
        </div>
        
        <div className="bg-card/50 rounded-lg p-3">
          <h5 className="text-xs font-semibold text-muted-foreground uppercase mb-1">Sentiment</h5>
          <div className={cn(
            "text-sm font-semibold",
            riskSentiment === 'risk_on' ? "text-[var(--trade-bullish)]" : riskSentiment === 'risk_off' ? "text-[var(--trade-bearish)]" : "text-foreground/80"
          )}>
      ...
      {vixLevel && (
        <div className="bg-card/50 rounded-lg p-3 mb-4">
          <div className="flex justify-between items-center">
            <span className="text-xs text-muted-foreground">VIX (Fear Index)</span>
            <span className={cn(
              "font-mono text-sm font-semibold",
              vixLevel > 30 ? "text-[var(--trade-bearish)]" : vixLevel > 20 ? "text-[var(--trade-neutral)]" : "text-[var(--trade-bullish)]"
            )}>
...
export function NewsInsights({
  ...
  return (
    <InsightPanel>
      <div className="mb-4">
        <h5 className="text-sm text-foreground/80 leading-relaxed">
          <span className="font-semibold">
            {sentimentLabel} Sentiment ({sentimentScore > 0 ? '+' : ''}{safeToFixed(sentimentScore, 2)})
          </span>
          {isContrarian && (
            <span className="text-[var(--trade-neutral)]"> is actually a bullish contrarian signal in this context.</span>
          )}
        </h5>
      ...
      {catalysts.length > 0 && (
        <div className="bg-card/50 rounded-lg p-3 mb-4">
          <h5 className="text-xs font-semibold text-foreground/90 mb-2">DETECTED CATALYSTS</h5>
          <div className="flex flex-wrap gap-1.5">
            {catalysts.map((catalyst, i) => (
              <Badge key={i} variant="outline" className="text-xs text-[var(--trade-neutral)] border-amber-400/50">
                {catalyst}
              </Badge>
            ))}
          </div>
      ...
      {topHeadlines.length > 0 && (
        <div className="mb-4">
          <h5 className="text-xs font-semibold text-foreground/90 mb-2">HEADLINE ANALYSIS</h5>
          <div className="bg-card/50 rounded-lg p-3">
            <p className="text-xs text-foreground/80 italic mb-2">"{topHeadlines[0]}"</p>
