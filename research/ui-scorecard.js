...
window.__uxScore = async function () {
  ...
  const unnamed = ctl.filter((c) => !((c.getAttribute('aria-label') || c.getAttribute('title') || c.textContent || c.querySelector('img[alt]')?.getAttribute('alt') || c.getAttribute('placeholder') || '').trim() || c.getAttribute('aria-labelledby') || c.closest('label')));
