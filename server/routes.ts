...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/portfolio-risk", async (_req, res) => {
    try {
      ...
      // Calculate total capital exposure (sum of entry prices * assumed position sizes)
      // For now, use defaultCapitalPerIdea from user prefs (10k account / $1k per idea = 10% per trade)
      const accountSize = 10000; // TODO: Get from user preferences
      const defaultCapitalPerIdea = 1000;
      
      const totalExposure = openIdeas.reduce((sum, idea) => {
        return sum + defaultCapitalPerIdea;
      }, 0);
      
      // Calculate total risk (sum of potential losses to stop losses)
      const totalRisk = openIdeas.reduce((sum, idea) => {
        const riskPerShare = Math.abs(idea.entryPrice - idea.stopLoss);
        const shares = Math.floor(defaultCapitalPerIdea / idea.entryPrice);
        const riskAmount = shares * riskPerShare;
        return sum + riskAmount;
      }, 0);
      
      // Calculate today's P&L (closed ideas from today only)
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const closedToday = allIdeas.filter(i => {
        if (!i.exitDate || i.outcomeStatus === 'open') return false;
        const exitDate = new Date(i.exitDate);
        exitDate.setHours(0, 0, 0, 0);
        return exitDate.getTime() === today.getTime();
      });
      
      const todayPnL = closedToday.reduce((sum, idea) => {
        return sum + (idea.realizedPnL || 0);
      }, 0);
      
      // Calculate max drawdown by building equity curve
      const closedIdeas = allIdeas.filter(i => i.outcomeStatus !== 'open' && i.exitDate);
      
      // Sort by exit date to build chronological equity curve
      const sortedTrades = closedIdeas
        .sort((a, b) => {
      ...
      // Build equity curve and track peak
      let runningBalance = accountSize;
      let peakAccountValue = accountSize;
      
      sortedTrades.forEach(idea => {
        runningBalance += (idea.realizedPnL || 0);
        if (runningBalance > peakAccountValue) {
          peakAccountValue = runningBalance;
        }
      });
      
      const currentAccountValue = runningBalance;
      const drawdown = ((currentAccountValue - peakAccountValue) / peakAccountValue) * 100;
      ...
      const maxDrawdown = -10; // -10% circuit breaker
      const circuitBreakerTriggered = drawdown <= maxDrawdown;
      ...
      res.json({
        ...
        openPositions: openIdeas.length,
        maxPositions: 10, // TODO: Make configurable
        todayPnL,
        todayPnLPercent: Math.round((todayPnL / accountSize) * 1000) / 10,
        currentAccountValue,
        drawdown: Math.round(drawdown * 10) / 10,
        circuitBreakerTriggered,
        tradingPaused: circuitBreakerTriggered,
      });
    } catch (error: any) {
