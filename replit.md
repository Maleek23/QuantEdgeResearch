...
### UI/UX Decisions
The UI features a Bloomberg-style dark theme with a consistent color palette (green for bullish, red for bearish, amber for neutral/warning, blue for primary actions). Typography uses Inter for UI and JetBrains Mono for financial data. Key features include real-time price displays, detailed analysis modals, mobile responsiveness, an intelligent advisory system, and advanced 3D Visual Analytics (Holographic Trading Floor, 3D Correlation Matrix Cube, 3D Brain Neural Network) using React Three Fiber and Three.js. Recent UI/UX improvements focus on readability, reduced visual clutter, and a streamlined Trade Desk with distinct sections for active and closed trades. A public-facing redesign enhances the landing page with prominent dual engine messaging.

**Simplified Navigation:**
- **Main Section**: Home, Trade Desk, Live Trading, Paper Trading, Trading Rules
- **More Section**: Performance, Market, Chart Analysis
- **System Section**: Pricing, Settings, Admin (conditional)
...
The frontend uses React/TypeScript, Tailwind CSS, Shadcn UI, TanStack Query, Wouter, and date-fns-tz. The backend uses Express.js with TypeScript, a PostgreSQL database (Neon) with Drizzle ORM, and Zod validation. Core features include a collapsible sidebar, symbol search, a metrics-focused dashboard, a unified trade ideas feed (Quant and AI generated), and a conversational QuantAI Bot with multi-provider fallback.

**NEW FEATURES ADDED (December 2025):**

1. **Paper Trading Simulator** - Virtual $100K portfolio with:
   - Trade execution from live trade ideas
   - Real-time P&L tracking and equity curves
   - Position management (open/close trades)
   - Win/loss record tracking
   - Settings for auto-execute and position sizing
   - 11 API endpoints for portfolio/position/equity management

2. **Wallet Tracker** - Cryptocurrency whale monitoring for ETH & Solana with:
   - Track multiple wallets across chains
   - Monitor holdings and token balances
   - Track large transfers (>$100K whale activity)
   - Alert system for suspicious activity
   - Supports both real data (Alchemy API) and mock data for demo
   - 10 API endpoints for wallet/transaction/alert management

3. **CT Tracker** - Crypto influencer intelligence with:
   - Track mentions from crypto influencer sources
   - Ticker extraction and sentiment analysis
   - Call performance tracking
   - Top trending tickers
   - Bullish/bearish breakdown
   - Mock data generation for testing
   - 8 API endpoints for sources/mentions/performance

**Dual Engine Architecture:**
- **Engine 1: AI Analysis** - Multi-provider AI (Claude, GPT, Gemini) for fundamental analysis, news catalysts, and market context
- **Engine 2: Quantitative Signals** - Research-backed engine using RSI(2), VWAP, volume spike detection, and chart pattern validation
- **Hybrid System** - Both engines work together: Quant finds technical setups, AI validates fundamental strength for precision + context
...
**Performance Optimization:** Tiered caching strategy with <60s end-to-end latency target for all trading decisions.
...
-   **Databento API:** Futures data (NQ, GC - real-time pricing, contract specs, CME data).
-   **Alchemy API (optional):** Ethereum/Solana whale wallet monitoring (if API key provided, otherwise uses mock data).
