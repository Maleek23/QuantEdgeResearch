...
- **Trade Ideas Feed:** Unified page with comprehensive filtering system: source filters (All Sources | AI | Quant | Manual) with visual badges (AI=Sparkles, Quant=BarChart3, Manual=Brain), asset type filters (All Assets | Stock Shares | Stock Options | Crypto), and direction filters (All | Long | Short | Day Trade). Header includes direct idea generation buttons (Generate Quant + Generate AI) for on-page creation. View toggle supports list (vertical) and grid (responsive 1/2/3 columns) layouts. Tabbed interface with date-based accordions for "NEW IDEAS," compact `TradeIdeaBlock` components, and detailed expandable views showing quantitative entry/target/stop levels, R:R, and catalyst summaries. Includes mini calendar for historical navigation.
