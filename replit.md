...
- **Options Price Fix (Critical):** Options were showing null prices because backend was excluding them from price fetching. Fixed by: (1) Adding `assetType === 'option'` to stockSymbols filter so options fetch underlying stock prices, and (2) Changing `ideas.find()` to `openIdeas.find()` to prevent finding closed ideas. All 8 option trade ideas now display live underlying stock prices.
- **Options Validation System (Critical):** Implemented comprehensive validation for all option trades to prevent data corruption and ensure correct P&L calculations. System validates four valid option combinations: {long call, short put} = bullish, {long put, short call} = bearish. Invalid option trades (e.g., long call with bearish target) are REJECTED with HTTP 400 error at POST /api/trade-ideas endpoint. Stock trades with direction/target mismatches are auto-corrected. All percent gain calculations now use `getNormalizedDirection()` helper to ensure P&L math aligns with option polarity. Fixed TSLA trade in database (direction: long, option_type: put). Validation preserves all valid combinations without corruption.
- **Options Timing Display Enhancement:** Updated EnhancedCountdown component to show full DATE and TIME for option exit windows (e.g., "Oct 24, 2:00 PM CST") alongside countdown timer. Options now display "Exit By:" with complete timestamp aligned with option expiry dates, providing clear visibility of trade timing for options strategies.
...
- **Date-Based Performance Filtering (Critical):** Implemented complete date filtering system for performance tracking. Backend now accepts `startDate` and `endDate` query parameters (YYYY-MM-DD format) for `/api/performance/stats` endpoint, using string-based date comparison to avoid timezone issues. Frontend Performance page features "Today" quick filter button and date range selector (Today, 7d, 30d, 3m, 1y, All) with per-filter cache keys. Enables daily accuracy tracking and performance analysis by specific time periods (e.g., Today: 13 ideas, 50% win rate, 18.3% quant accuracy).
- **Performance Page Date Filtering Consistency (Critical):** Resolved critical bug where "All Time" filter showed "Today's Performance" card and frontend data calculations (closedIdeas, filteredIdeas, advancedMetrics) were inconsistent with backend stats. Fixed by: (1) Adding missing `case 'today': return startOfDay(now)` to rangeStart calculation to ensure closedIdeas respects selected date range, (2) Changing "Today's Performance" card condition from `dateRange === 'all'` to `dateRange === 'today'` so it only shows when explicitly selecting Today filter, (3) Adding prominent "All Time Performance" header when viewing all-time data. All performance metrics now use consistently filtered data across stats, closedIdeas, advancedMetrics, and all visualization sections.
