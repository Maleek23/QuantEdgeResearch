...
- **isPremium Middleware (server/replitAuth.ts):** Enforces premium-only access by verifying both subscriptionTier (premium/pro/admin) AND subscriptionStatus === "active" (admins bypass status check)
- **Protected Endpoints:** POST /api/quant/generate-ideas, POST /api/ai/generate-ideas, POST /api/ai/chat, POST /api/ai/parse-chat-idea require active premium subscription
- **Free Access Preserved:** GET endpoints for viewing trade ideas, performance stats, market data, and watchlists remain public for free tier users
- **Error Responses:** 401 "Unauthorized" for unauthenticated users, 403 "Premium subscription required" or "Active subscription required" for insufficient access
