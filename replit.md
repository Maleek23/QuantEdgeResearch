...
### Key Features
- **Collapsible Sidebar Navigation:** Professional sidebar with icon-only collapse mode organizing platform into sections (Dashboard, Trade Ideas, Watchlist, About). Features toggle button in header, anchor navigation to dashboard sections, smooth expand/collapse transitions, and logo that shows icon-only when collapsed. Updated October 2025 to remove non-existent sections (Risk Calculator, Screener, Performance) and add About section.
- **About Section:** Professional creator profile section featuring photo, bio, education (M.S. Systems Engineering - University of Oklahoma, B.S. Computer Engineering - UT Arlington), certifications (CISA, MATLAB, Simulink), and social links (LinkedIn, GitHub). Highlights experience in AI/ML model validation and risk analytics at DTCC.
