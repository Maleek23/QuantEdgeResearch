...
The quantitative engine leverages three academically-proven signals: RSI(2) Mean Reversion with a 200-Day MA Filter, VWAP Institutional Flow, and Volume Spike Early Entry. It incorporates ADX Regime Filtering, Signal Confidence Voting (requiring 2+ signals), and an Optimal Trading Window filter (9:30-11:30 AM ET). A Hybrid AI+Quant system combines quantitative signals with AI fundamental analysis. AI-generated trades are subject to a four-layer risk validation framework enforcing a maximum 5% loss cap, minimum 2:1 R:R ratio, price sanity checks, and an extreme volatility filter. All generation methods prevent duplicate trades.
